...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var results = samplesArray.filter(obj => obj.id == sample);
    var filteredMetadata = metadataArray.filter(sampleObj => sampleObj.id == sample);
    //  5. Create a variable that holds the first sample in the array.
    var samplea = results[0];

    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    var otuIDs = samplea.otu_ids;
    var otuLabs = samplea.otu_labels;
    var sampleVals = samplea.sample_values; 
